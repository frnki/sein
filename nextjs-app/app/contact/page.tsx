...
export default function ContactPage() {
  ...
  const activeComponent = CONTACT_TABS.flatMap((group) => group.items).find(
  ...
  return (
    <div className="space-y-4 border-t pt-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-sm font-medium">02-877-8811</p>
          <p className="text-xs text-gray-500">평일 9:00 - 18:00</p>
        </div>
      </div>
      <div className="flex items-center gap-3">
        <Mail className="w-4 h-4" />
        <div>
          <p className="text-sm font-medium">seined@naver.com</p>
...
function LocationSection() {
  const handleLocationClick = (type: "headquarters" | "factory") => {
    const urls = {
      headquarters:
        "https://map.naver.com/p/search/%EC%98%81%EB%93%B1%ED%8F%AC%EB%A1%9C5%EA%B8%B8%2019",
      factory:
        "https://map.naver.com/p/search/%ED%8C%8C%EC%A3%BC%EC%8B%9C%20%EC%86%8C%EB%9D%BC%EC%A7%80%EB%A1%9C%2063-25",
    };
      ...
      >
        {CONTACT_TABS.map((group, index) => (
          <div key={index} className="space-y-1">
            <div className="px-4 py-2 text-[16px] font-bold">{group.title}</div>
            {group.items.map((tab) => (
              <TabButton
                key={tab.id}
                id={tab.id}
